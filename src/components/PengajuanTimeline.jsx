  ...
  if (needsRevision) {
    return (
      <div className="rounded-lg border border-amber-300 bg-amber-50 p-4">
        <p className="font-semibold text-amber-800">Perlu Revisi</p>
  ...
  if (isRejected) {
    return (
      <div className="rounded-lg border border-red-300 bg-red-50 p-4">
        <p className="font-semibold text-red-800">Pengajuan Ditolak</p>
      </div>
    );
  }

  return (
    <ol className="relative border-s border-gray-200 ms-4 space-y-0">
      {STATUS_FLOW.map((step, idx) => {
        ...
        return (
          <li key={step.key} className="mb-8 ms-6 last:mb-2">
            <span
              className={`absolute -start-3 flex h-6 w-6 items-center justify-center rounded-full ring-4 ring-white text-xs font-bold ${
