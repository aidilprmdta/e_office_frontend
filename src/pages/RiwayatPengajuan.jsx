...
import { Search, Plus, Eye, Trash2, X, FileText, Calendar, Hash, Edit3, UploadCloud, Download } from 'lucide-react';
import {
  formatDate,
  normalizeStatus,
  canDeletePengajuan,
...
export default function RiwayatPengajuan() {
  ...
  useEffect(() => {
    ...
    if (letter) openDetail(letter);
    setSearchParams({}, { replace: true });
  }, [letters, searchParams]);
  ...
  const columns = [
    {
      ...
      render: (date) => <span className="font-medium text-gray-700 dark:text-gray-300">{formatDate(date)}</span>,
    },
    {
      key: 'jenis_pengajuan',
      label: 'Jenis',
      ...
      render: (id, row) => (
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            className="p-1.5 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 text-blue-600 rounded-md"
          ...
          {isRevisiStatus(row.status) && (
            <button
              ...
              className="p-1.5 bg-amber-50 dark:bg-amber-900/30 hover:bg-amber-100 text-amber-700 rounded-md"
              title="Edit / Revisi"
            >
              <Edit3 size={16} />
            </button>
          ...
          {canDeletePengajuan(row.status) && (
            <button
              ...
              className="p-1.5 bg-red-50 dark:bg-red-900/30 hover:bg-red-100 text-red-600 rounded-md"
              title="Batalkan"
            >
              <Trash2 size={16} />
            </button>
  ...
  const inputClass =
    'w-full px-4 py-2 border-2 border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50 dark:bg-gray-800 dark:text-gray-100 focus:bg-white dark:focus:bg-gray-800';
      ...
      <div className="space-y-6">
        ...
        <div className={panelClass}>
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">Belum ada riwayat pengajuan.</div>
          )}
        </div>
      </div>
      ...
      <AnimatePresence>
        {showCreateModal && (
          <ModalShell onClose={() => setShowCreateModal(false)}>
            <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2 mb-4">
      ...
      <AnimatePresence>
        {showRevisiModal && selectedLetter && (
          <ModalShell onClose={() => setShowRevisiModal(false)}>
            <h3 className="text-xl font-bold text-amber-800 dark:text-amber-200 mb-2">Edit / Revisi Pengajuan</h3>
            {selectedLetter.catatan_revisi && (
              <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-lg text-sm text-amber-900 dark:text-amber-100">
          ...
          <ModalShell onClose={() => setShowDetailModal(false)} wide>
            ...
            <div className="space-y-4">
              ...
              {selectedLetter.catatan_dosen && (
                <div className="p-3 bg-red-50 dark:bg-red-950/30 border border-red-100 dark:border-red-900 rounded-lg text-sm text-red-800 dark:text-red-200">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className={`bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full p-6 relative z-10 max-h-[90vh] overflow-y-auto border border-gray-100 dark:border-gray-800 ${wide ? 'max-w-2xl' : 'max-w-xl'}`}
      >
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-1 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full text-gray-400"
        >
          <X size={24} />
        </button>
        {children}
  ...
  return (
    <div>
      ...
      <div
        ...
        className="w-full border-2 border-dashed border-blue-300 dark:border-blue-700 rounded-xl p-6 flex flex-col items-center bg-blue-50 dark:bg-blue-950/30 hover:bg-blue-100 dark:hover:bg-blue-950/50 cursor-pointer"
      >
